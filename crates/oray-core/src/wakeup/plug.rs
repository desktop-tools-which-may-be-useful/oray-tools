...
/// (space becomes `+`, matching `reqwest`'s `.form()` encoding).
...
/// Reconstruct the encoded request body `reqwest` would produce for a
/// `.form()` request, so it can be captured in the request trace.
